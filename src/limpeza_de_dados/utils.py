...
def split_coordinates(df: pd.DataFrame,
...
def assign_nest_id(df: pd.DataFrame,
    ...
    df[new_id_col] = df.groupby([lat_col, lon_col])[register_number_col].transform('min').astype('Int64')
    return df

def count_values_by_label(df: pd.DataFrame,
                          label_col: str,
...
def find_new_entries (df_raw: pd.DataFrame,
                     df_analysis: pd.DataFrame,
                     unique_col: str = 'Timestamp'):
    return df_raw[~df_raw[unique_col].isin(df_analysis[unique_col])]

def replace_values_in_col(df: pd.DataFrame,
                          col: str = 'Estrutura de nidificação',
                          replacement: tuple[str, str] = (
                              'Edifício de apoio (arrecadação, garagem, etc.)', 'Ed. de apoio')) -> pd.DataFrame:
...
def trim_empty_spaces (df: pd.DataFrame, cols_to_trim:list[str]|None = None):
    ...
    df[cols_to_trim] = df[cols_to_trim].apply(lambda x: x.str.strip() if x.dtype == "object" else x)
    return df
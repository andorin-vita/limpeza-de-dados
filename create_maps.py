...
def get_region_options(yaml_path: str, df: pd.DataFrame, col: str) -> list[str]:
    ...
    return sorted(list(set(from_df) | set(from_yaml)))
...
def create_map_sidebar(df: pd.DataFrame,
    ...
    with st.sidebar:
        with st.form("my_form"):
            species: list[str] = sorted(df[species_col].dropna().unique())
            nest_structure: list[str] = sorted(df[nest_structure_col].dropna().unique())
